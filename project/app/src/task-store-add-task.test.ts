...
describe('task store addTask action', () => {
  ...
  it('adds tasks and rebuilds derived maps', () => {
    ...
    expect(nextState.tasksByStatus[TaskStatus.BACKLOG].map((task) => task.id)).toEqual(
      ['task-1'],
    )
    expect(nextState.tasksByStatus[TaskStatus.TODO].map((task) => task.id)).toEqual(['task-2'])
    expect(nextState.tasksByParent['task-1'].map((task) => task.id)).toEqual(['task-2'])
  })
