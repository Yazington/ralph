...
describe('Shadcn/UI Setup', () => {
  describe('Configuration Files', () => {
    it('should have components.json file with correct configuration', () => {
      ...
      expect(existsSync(componentsJsonPath), 'components.json should exist').toBe(true)
      ...
      expect(componentsJson.iconLibrary, 'Icon library should be lucide').toBe('lucide')
    })

    it('should have correct alias configuration in tsconfig.json', () => {
      const tsconfigJsonPath = resolve(__dirname, '../tsconfig.json')
      expect(existsSync(tsconfigJsonPath), 'tsconfig.json should exist').toBe(true)
      ...
      expect(tsconfigJson.compilerOptions.paths, 'paths should exist').toBeDefined()
      expect(tsconfigJson.compilerOptions.paths['@/*'], '@/* alias should point to ./src/*').toEqual(['./src/*'])
    })
    ...
    it('should have correct alias configuration in tsconfig.app.json', () => {
      ...
      expect(existsSync(tsconfigAppJsonPath), 'tsconfig.app.json should exist').toBe(true)

      const tsconfigAppJsonContent = readFileSync(tsconfigAppJsonPath, 'utf-8')
      expect(tsconfigAppJsonContent, 'tsconfig.app.json should contain baseUrl').toContain('"baseUrl": "."')
      expect(tsconfigAppJsonContent, 'tsconfig.app.json should contain paths').toContain('"paths"')
      expect(tsconfigAppJsonContent, 'tsconfig.app.json should contain @/* alias').toContain('"@/*":')
    })
    ...
    it('should have alias configuration in vite.config.ts', () => {
      ...
      expect(viteConfig, 'vite.config.ts should contain path alias').toContain('"@": path.resolve(__dirname, "./src")')
    })
  })
  ...
  describe('Utility Files', () => {
    it('should have lib/utils.ts with cn function', () => {
      ...
      expect(utils, 'utils.ts should export cn function').toContain('export function cn')
      expect(utils, 'utils.ts should import clsx').toContain('from "clsx"')
      expect(utils, 'utils.ts should import tailwind-merge').toContain('from "tailwind-merge"')
    })
  })
  ...
  describe('Dependencies', () => {
    it('should have class-variance-authority in dependencies', () => {
      ...
      expect(packageJson.dependencies['class-variance-authority'], 'class-variance-authority should be installed').toBeDefined()
    })

    it('should have clsx in dependencies', () => {
      const packageJsonPath = resolve(__dirname, '../package.json')
      const packageJson = JSON.parse(readFileSync(packageJsonPath, 'utf-8'))
      expect(packageJson.dependencies['clsx'], 'clsx should be installed').toBeDefined()
    })
    ...
    it('should have tailwind-merge in dependencies', () => {
      const packageJsonPath = resolve(__dirname, '../package.json')
      const packageJson = JSON.parse(readFileSync(packageJsonPath, 'utf-8'))
      expect(packageJson.dependencies['tailwind-merge'], 'tailwind-merge should be installed').toBeDefined()
    })

    it('should have tw-animate-css in devDependencies', () => {
      const packageJsonPath = resolve(__dirname, '../package.json')
      const packageJson = JSON.parse(readFileSync(packageJsonPath, 'utf-8'))
      expect(packageJson.devDependencies['tw-animate-css'], 'tw-animate-css should be installed').toBeDefined()
    })
  })
  ...
  describe('CSS Variables', () => {
    it('should have CSS variables in index.css', () => {
      ...
      expect(indexCss, 'index.css should have CSS variables').toContain('--background:')
      expect(indexCss, 'index.css should have foreground variable').toContain('--foreground:')
      expect(indexCss, 'indexCss should have primary variable').toContain('--primary:')
      expect(indexCss, 'indexCss should have border variable').toContain('--border:')
    })

    it('should have shadcn/ui @theme inline section', () => {
      const indexCssPath = resolve(__dirname, '../src/index.css')
      const indexCss = readFileSync(indexCssPath, 'utf-8')
      expect(indexCss, 'index.css should have @theme inline section').toContain('@theme inline')
    })
  })
})
