...
describe('Shadcn/UI Setup', () => {
  describe('Configuration Files', () => {
    ...
    it('should have alias configuration in vite.config.ts', () => {
      ...
      expect(viteConfig, 'vite.config.ts should contain path alias').toMatch(
        /@['"]?\s*:\s*path\.resolve\(__dirname,\s*['"]\.\/src['"]\)/
      )
    })
  })
  ...
  describe('Utility Files', () => {
    it('should have lib/utils.ts with cn function', () => {
      ...
      expect(utils, 'utils.ts should import clsx').toMatch(/from ['"]clsx['"]/)
      expect(utils, 'utils.ts should import tailwind-merge').toMatch(
        /from ['"]tailwind-merge['"]/
      )
    })
  })
